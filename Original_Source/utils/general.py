...
def non_max_suppression(prediction,
    ...
    time_limit = 120.0 + 20 * bs  # seconds to quit after
    #time_limit = 0.3 + 0.03 * bs  # seconds to quit after # WARNING: NMS time limit 0.900s exceeded
...
def print_mutation(results, hyp, save_dir, bucket, prefix=colorstr('evolve: ')):
    ...
    with open(evolve_csv, 'a') as f:
        f.write(s + ('%20.7g,' * n % vals).rstrip(',') + '\n')

    # Save yaml
    with open(evolve_yaml, 'w') as f:
        data = pd.read_csv(evolve_csv,dtype=float, skipinitialspace=True) # TODO: upload to yolo5gituhub
        data = data.rename(columns=lambda x: x.strip())  # strip keys
        i = np.argmax(fitness(data.values[:, :4]))  #
        generations = len(data)

        for j in range(7):
            print(data.values[i,j], type(data.values[i,j]))
            if isinstance(data.values[i,j],str):
                data.iloc[i,j] = float(data.values[i,j].strip())
            if isinstance(data.values[i,j],list):
                data.iloc[i,j] = float(str(data.values[i,j]).strip())
            if np.isnan(data.values[i, j]):
                data.iloc[i, j] = -1.0
            else:
                data.iloc[i, j] = float(data.values[i, j])
        
        f.write('# YOLOv5 Hyperparameter Evolution Results\n' + f'# Best generation: {i}\n' +
                f'# Last generation: {generations - 1}\n' + '# ' + ', '.join(f'{x.strip():>20s}' for x in keys[:7]) +
                '\n' + '# ' + ', '.join(f'{x:>20.7g}' for x in data.values[i, :7]) + '\n\n')
        yaml.safe_dump(data.loc[i][7:].to_dict(), f, sort_keys=False)

    # Print to screen
    LOGGER.info(prefix + f'{generations} generations finished, current result:\n' + prefix +
                ', '.join(f'{x.strip():>20s}' for x in keys) + '\n' + prefix + ', '.join(f'{x:20.7g}'
                                                                                         for x in vals) + '\n\n')
